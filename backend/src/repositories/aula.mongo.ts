...
export class AulaMongoRepository {
  ...
  async findByIds(ids: string[]) {
    ...
    const aulas = await this.collection().find({ _id: { $in: objectIds } } as any).toArray();
    return aulas.map(a => ({ ...a, _id: String((a as any)._id) }));
  }
  ...
  //coordenador listar todas as aulas
  async listAll() {
    return this.collection()
      .find({} as any)
      .sort({ createdAt: -1 } as any)
